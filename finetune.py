# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 18:45:40 2018

fine-tune脚本 对应文件夹是sen2,acc=73.4%

fine-tune之后 acc=84.2%
@author: freeze
"""
...
sess=tf.Session()
saver=tf.train.import_meta_graph('sen2/model.ckpt-958.meta')
saver.restore(sess,tf.train.latest_checkpoint('sen2/'))
...
while(dl.epoch>0):
    ...
    if (dl.epoch>0):
        ...
        if (b>max_acc):
            max_acc=b
            saver.save(sess,'fine-tune-sen2/model.ckpt',global_step=iteration)
        train_acc.append(a)
        test_acc.append(b)
